...
const skillGroups = [
  ...
  { title: "Databases", icon: <FaDatabase />, skills: [["PostgreSQL", <SiPostgresql />], ["MySQL", <SiMysql />], ["MongoDB", <SiMongodb />]] },
  { title: "Deployment & Tools", icon: <FaCode />, skills: [["Git", <SiGit />], ["GitHub", <FaGithub />], ["Docker", <FaDocker />], ["Firebase", <SiFirebase />], ["Vercel", <SiVercel />]] },
] as const;

export function Skills() { const reducedMotion = useReducedMotion(); return <section id="skills" className="section section-muted"><div className="container"><SectionHeading eyebrow="05 / Skills" title="Primary technologies for product-focused development.">React, FastAPI, and Spring Boot are supported by practical database, deployment, and collaboration tools.</SectionHeading><div className="grid gap-4 sm:grid-cols-2">{skillGroups.map((group, index) => <motion.article className="glass-card p-6" key={group.title} initial={reducedMotion ? false : { opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ delay: index * 0.06, duration: 0.35 }}><div className="flex items-center gap-3 text-[var(--accent)]"><span className="text-xl">{group.icon}</span><h3 className="font-semibold text-[var(--text)]">{group.title}</h3></div><ul className="mt-5 flex flex-wrap gap-2">{group.skills.map(([name, icon]) => <li className="skill-pill" key={name}><span>{icon}</span>{name}</li>)}</ul></motion.article>)}</div></div></section>; }
